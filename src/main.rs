...
async fn main() -> anyhow::Result<()> {
    dotenv::dotenv().ok();
    let whitelist_rpc_url =
        std::env::var("WHITELIST_RPC_URL").unwrap_or("0.0.0.0:8080".to_string());
    println!(
        "NYKS TX DECODER RPC SERVER RUNNING ON {}",
        whitelist_rpc_url
    );
    let server = ServerBuilder::default().build(whitelist_rpc_url).await?;
